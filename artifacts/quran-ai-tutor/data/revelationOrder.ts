...
export function sortByRevelation<T extends { number: number }>(surahs: T[]): T[] {
  const orderMap = new Map(REVELATION_ORDER.map((surahNum, idx) => [surahNum, idx]));
  return [...surahs].sort(
    (a, b) => (orderMap.get(a.number) ?? 999) - (orderMap.get(b.number) ?? 999),
  );
}
